 ...
 *
 * Ported from the legacy backend (src/models/permissions.ts) unchanged in
 * substance — this module is pure TypeScript with no runtime dependencies, so
 * it moves onto the Workers/Drizzle stack as-is.
 *
 * Two role planes coexist:
 *  - `Role.SUPER_ADMIN` is a platform-level role stored on `users.systemRole`.
 *  - The remaining roles (owner/admin/member/viewer) are per-organization and
 *    stored on `user_organizations.role`.
 * Effective permissions for a request are resolved in AuthorizationService by
 * unioning the system-role permissions (if any) with the active org-role
 * permissions.
 */
...
/** Roles assignable to an organization membership (excludes the platform role). */
export const ORG_ROLES = [Role.ORG_OWNER, Role.ORG_ADMIN, Role.ORG_MEMBER, Role.ORG_VIEWER] as const
...
/** Role hierarchy for permission inheritance (a role inherits every listed role's perms). */
...
/** True if the role is directly granted the permission (no hierarchy expansion). */
export function roleHasPermission(role: Role, permission: Permission): boolean {
  return ROLE_PERMISSIONS[role].includes(permission)
}

/** All permissions for a role, expanding the role hierarchy. */
...
/** True if role1 sits at or above role2 in the hierarchy. */
